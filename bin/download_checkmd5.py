...
import urllib.request, urllib.parse, urllib.error, hashlib, sys, os
...
def main():
    ...
    if not os.path.exists(dest):
        print("Downloading from %s to %s..."%(uri, dest), end=' ')
        sys.stdout.flush()
    ...
    if md5sum:
        print("Computing md5sum on downloaded file", end=' ')
        sys.stdout.flush()
        ...
        if checksum!=md5sum:
            if not fresh:
                print("Checksum mismatch (%s != %s), re-downloading file %s"%(checksum, md5sum, dest), end=' ')
                sys.stdout.flush()
                os.remove(dest)
                urllib.request.urlretrieve(uri, dest)
                print("done")
